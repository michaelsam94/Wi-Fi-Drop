...
import com.example.core.domain.TransferItem
import com.example.core.domain.FolderEntry
import io.ktor.http.*
...
import com.example.core.domain.WebShareDownloadEvent
...
class KtorLocalServer(
    ...
    suspend fun start(port: Int, shareItems: List<TransferItem>): Int {
        val createdServer = embeddedServer(Netty, port = port) {
            routing {
                ...
                get("/file/{index}") {
                    ...
                    if (item is TransferItem.SingleFile) {
                        ...
                        call.respondOutputStream(ContentType.Application.OctetStream) {
                            val resolver: ContentResolver = this@KtorLocalServer.context.contentResolver
                            try {
                                resolver.openInputStream(Uri.parse(item.uriString))?.use { input ->
                ...
                get("/folder_entry/{index}/{entryIndex}") {
                    ...
                    call.respondOutputStream(ContentType.Application.OctetStream) {
                        try {
                            this@KtorLocalServer.context.contentResolver.openInputStream(Uri.parse(entry.uriString))?.use { input ->
